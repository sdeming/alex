/* ex3.c - Display list example.
 *
 * Demonstrates using multiple display lists.
 *
 * If you noticed in ex2, the mouse would go to the bottom if you brought
 * the two images to the front.  Using another display list for items that
 * need to stay on top is a good solution.
 *
...
int on_lclick(display_item *pDI, int x, int y)
{
	bring_window_to_top(pDI);

	return 0;
}

int on_rclick(display_item *pDI, int x, int y)
{
	send_window_to_bottom(pDI);
...
void main(int argc, char *argv[])
{
	...
	dl_gui = create_display_list(&light_table, &trans_table);
	diWinFrom = add_display_item(dl_gui, create_display_item(winFrom, DISPLAY_WINDOW, f_x, f_y, 1, 0, 1));
	diWinTo = add_display_item(dl_gui, create_display_item(winTo, DISPLAY_WINDOW, t_x, t_y, 1, 0, 1));
	diImgLight = add_display_item(dl, create_display_item(imgLight, DISPLAY_LIGHT, mouse_x, mouse_y, 1, 0, 1));

	// set gui display list.
	gui_setdisplaylist(dl_gui);
	winFrom->on_mouselclick = on_lclick;
	winFrom->on_mouserclick = on_rclick;
	winTo->on_mouselclick = on_lclick;
	winTo->on_mouserclick = on_rclick;
	...
	while (!kbhit())
	{
		...
		set_all_windows_styles(dl_gui, -1, WD_TRANSLUCENT);

		if ((diTop = get_top_window(dl_gui, mouse_x, mouse_y)) != NULL)
		...
		if (mouse_b & 1)
		{
			...
			if (click)
			{
				bring_window_to_top(get_top_window(dl_gui, mouse_x, mouse_y));
				click = 0;
			}
		}
		*/
		...
		render_display_list(dl_gui, scrFlip, NULL); // 1st
		render_display_list(dl, scrFlip, NULL); // 2nd
		blit(scrFlip, screen, 0, 0, 0, 0, SCREEN_W, SCREEN_H);

		// Restore original background in off screen buffer.
		// note that undo_render must be done in the opposite order than render.
		undo_render_display_list(dl, scrFlip, NULL); // 2nd
		undo_render_display_list(dl_gui, scrFlip, NULL); // 1st
